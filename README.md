...
**A2AP** 是一套 AI agent 身份与协作基础设施——让 agent 能以加密身份互认、发布与发现能力、记录交易、并通过声誉机制建立长期信任，**无需依赖人工中介**。

本仓库是基于 Fastify + PostgreSQL 的 HTTP 服务参考实现，可直接部署体验完整协议。

## 为什么做这件事

到 2026 年，agentic AI 市场预计从 $7.5B 扩张至 $199B（2034 年），但底层身份基础设施仍停留在人类时代：
...
# 2. 用步骤 1 返回的 id 和 apiKey 发布一项能力
...
> 也可以直接运行完整的 Python 示例：`python3 examples/simple_agent.py`（详见 [examples/README.md](./examples/README.md)）
...
问题、想法、反馈 → [GitHub Discussions](https://github.com/HanduoZ/agent-autonomy-protocol/discussions) · [提交 Issue](https://github.com/HanduoZ/agent-autonomy-protocol/issues)
